...
bool Player::animationPlayer(Engine::ClusterName cluster, Engine::Entity once)
{
    ...
    bool stat = GET_COMP_M.hasComponent<Component::EntityMask>(once)
        && GET_COMP_M.get<Component::EntityMask>(once)._currentMask == Component::MASK::ENEMY;
...
Player::Player(
    ClusterName cluster, int playerNumber, const vector2D &position, const vector2D &velocity, bool isLocalPlayer)
{
    ...
    componentManager.add<Engine::Hitbox>(
        entity, hitboxSize.x, hitboxSize.y, [cluster](Engine::Entity a, Engine::Entity b) {
...
void Player::configAppearance(
    Entity entity, vector2D const &position, Engine::ComponentManager &componentManager, int playerNumber)
{
    ...
    chargeShot->setOffsetPosition(
        vector2D(playerEntitySize.x - shotEntitySize.x / 2, playerEntitySize.y / 2 - shotEntitySize.y / 2));
