...
namespace Network
{
    ...
    class AsioClientTCP : public AsioConnectionTCP<PACKETSIZE> {
      public:
        ...
        bool connect(const std::string &ip, const std::size_t port) override
        {
            ...
            auto newConnection(std::make_shared<tcp::socket>(
                AAsioConnection<PACKETSIZE>::_ioContext));
            ...
            try {
                ...
                if (_ping.count()
                    > _connectionTimer.getElapsedTime().count()) {
                    std::this_thread::sleep_for(
                        _ping - _connectionTimer.getElapsedTime());
