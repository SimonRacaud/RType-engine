...
CreateEntityRequest::CreateEntityRequest(size_t roomId, uint32_t id, string entityType, Time timestamp,
    ...
    : Tram::Serializable(Tram::TramType::CREATE_ENTITY_REQUEST, sizeof(CreateEntityRequest)), roomId(roomId), id(id),
      timestamp(timestamp), port(0), position(position), velocity(velocity)
{
    if (entityType.empty()) {
        this->entityType[0] = '\0';
    } else {
...
CreateEntityRequest::CreateEntityRequest(size_t roomId, uint32_t id, string entityType, Time timestamp, size_t port,
    std::string const &ip, netVector2f const &position, netVector2f const &velocity)
    : Tram::Serializable(Tram::TramType::CREATE_ENTITY_REQUEST, sizeof(CreateEntityRequest)), roomId(roomId), id(id),
      timestamp(timestamp), port(port), position(position), velocity(velocity)
{
    if (entityType.empty()) {
        this->entityType[0] = '\0';
    } else {
...
CreateEntityRequest::CreateEntityRequest(
    size_t roomId, string entityType, Time timestamp, netVector2f const &position, netVector2f const &velocity)
    : Tram::Serializable(Tram::TramType::CREATE_ENTITY_REQUEST, sizeof(CreateEntityRequest)), roomId(roomId), id(-1),
      timestamp(timestamp), port(0), position(position), velocity(velocity)
{
    this->ip[0] = '\0';
    if (entityType.empty()) {
